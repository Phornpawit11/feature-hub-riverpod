...
void main() {
  group('AuthRemoteDatasource', () {
    test('signInWithEmailPassword sends login request and parses session', () async {
    ...
    test('signInWithGoogle sends id token request and parses session', () async {
      late RequestOptions capturedOptions;
      final dio = Dio(
        BaseOptions(baseUrl: 'http://localhost:3000/api'),
      )..interceptors.add(
      ...
      final session = await datasource.signInWithGoogle(idToken: 'google-token');
    ...
    test('maps string error message to AuthException', () async {
      final dio = Dio(
        BaseOptions(baseUrl: 'http://localhost:3000/api'),
      )..interceptors.add(
          InterceptorsWrapper(
    ...
    test('maps validation error list to joined AuthException message', () async {
      final dio = Dio(
        BaseOptions(baseUrl: 'http://localhost:3000/api'),
      )..interceptors.add(
          InterceptorsWrapper(
      ...
      await expectLater(
        () => datasource.signInWithEmailPassword(email: 'bad-email', password: ''),
        throwsA(
          isA<AuthException>().having(
            (error) => error.message,
            'message',
